  ...
  void _calculateResult() {
    try {
      String sanitizedEquation =
          screenText.replaceAll('x', '*').replaceAll('÷', '/');
      Parser p = Parser();
  ...
  void _toggleSign() {
    setState(() {
      if (screenText.isNotEmpty) {
        if (screenText[0] == '-') {
          screenText = screenText.substring(1); // Hapus tanda negatif
        } else {
          screenText = '-' + screenText; // Tambahkan tanda negatif
        }
      }
    });
  }
